...
5. `renderChart(times, precip)` is called at the end of `render()` and draws the Chart.js bar chart into `<canvas id="precipChart">`. A module-level `precipChart` variable holds the instance so it can be destroyed and recreated on re-search.
...
**Chart:** Chart.js is loaded from CDN (no install). The bar chart uses two datasets: per-bar colors (blue `#4299e1` for dry, red `#fc8181` for wet) and a constant line dataset at 15 for the dashed threshold. Y axis is fixed 0–50% in 5% steps.

**Time handling:** Open-Meteo returns ISO timestamps in the location's local time (no `Z` suffix). To avoid browser timezone shifts, all display formatting appends `Z` to the ISO string and passes `timeZone: 'UTC'` to `toLocaleTimeString` — this ensures times reflect the searched location, not the user's system clock. `formatHour()` and `getWindowEndLabel()` both accept a `todayStr` (`"YYYY-MM-DD"`) and append `(tomorrow)` to any time that falls on the next calendar day.
